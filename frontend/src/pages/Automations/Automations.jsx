...
import {
  ...
  FileText, Globe, Image, Tag, Smile, Copy, Webhook, Activity,
  MoreVertical, AlertCircle, CheckCircle, ToggleLeft, ToggleRight
} from 'lucide-react'
...
function AutomationModal({ automation, onClose, onSave }) {
  const [form, setForm] = useState({ name: automation?.name || '', description: automation?.description || '', trigger_type: automation?.trigger_type || 'manual' })
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const payload = { ...form, trigger_config, steps: parsedSteps }
...
function AutomationCard({ automation, onEdit, onDelete, onToggle, onRun, onDuplicate }) {
  ...
  return (
    <div style={{
      ...
      {/* Action row */}
      <div style={{ display: 'flex', gap: 6, borderTop: '1px solid var(--border-primary)', paddingTop: 10 }}>
